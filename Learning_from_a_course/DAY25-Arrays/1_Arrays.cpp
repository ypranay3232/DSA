...
Assume that we want to find the maximum of 3 numbers we declare and do a if else condition but what about 300,3000,30,000 etc. (.i.e. as the input numbers increases we need a way to store them so we use arrays)
...
int main()
{
    ...
    // Why this Doesn't print anything is because for character : Cout has a special overload (char*) it assumes it's a c-string. It assumes to print every character in memory until it finds a null terminator('\0')
    // Because the array is uninitialized (not assigned any values) it contains garbage values (so garbage value may be found out as 0X3491 so it stopped printing)

    // (.i.e. when a char aray is passed it points to char* pointer the ostream overload for char* treats it as a null terminated ('\0') c-string. since array is uninitialized it contains garbage values so it prints nothing)
    char array2[166] = {};
    // for verification we see the address if its starting with 0 then it's not going to print
    cout << "The address of char is : " << &array2 << "Because its starting with 0 its not going to print" << endl;
    cout << "The character array is : " << array2 << endl;

    bool array3[23] = {};
    cout << "The Boolean array is : " << array3 << endl;

    // Array initializatin : can be done in two ways 1)create an array and dynamically add values to it 2)define an array ize and add values according to the size.
