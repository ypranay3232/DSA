...
int main()
{
    // lets see how to print a half pyramind ?
    /*
    ...
    */
    ...
    for (int rows = 0; rows < No_Of_Rows; rows += 1)
    {
        ...
        {
            // first outer loop goes to inner loop with value '0' inner loop cols =0. and condition == cols<rows+1 (cols=0,rows=rows+1) so it works as 0<1 yes so the conditon became true so the * prints)
            cout << "*";
        }
        cout << endl;
    }
    ...
    for(int rows=0; rows<No_Of_Stars;rows+=1){ //first lets create rows
        for(int cols=0; cols<No_Of_Stars-rows;cols+=1){//now here chek the condition if cols<input-rows the logic is (input-rows) so it prints a descending order.
            cout<<"*";
        }
        cout<<endl;
    }
