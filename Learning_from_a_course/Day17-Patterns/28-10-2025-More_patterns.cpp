...
int main(){
    ...
    for(int row = 1; row <= n; row = row + 1){
        ...
        for(int cols = 1; cols <= n; cols = cols + 1){
            ...
            // Prints 5 at each last column so we use cols == n( cols 0,1,2,3,4 == 5) so when we print cols we print them as (2 5), (3 5), (4 5) at their respective positions so it looks like a (N-1) decrementing pattern for 5.
            if(row == 1 || cols == row || cols == n){
                cout << cols << " ";
                // The else if prints two spaces inbetween the diagonals : cols > row == prints after diagonal (i.e 5), cols<n prits before diagonal (i.e 5)
            } else if(cols > row && cols < n) {
                cout << "  ";  // Only print spaces BETWEEN the diagonal and last column
            }
        }
        cout << endl;
    }
