...
int main()
{
    ...
    // Boolean Data type. It contains just two values either true or false. And it takes 1 Byte of memory (8 bits) but as we understood 1 bit is enought to represent (0/1) true of false. so why are we using 8 bits(1byte) becuase in computer the minimum addressable memory is 1 byte so you can't use a memory smaller than 1 Byte.
    bool Access = true;
    ...
    // if both works the same what's the difference Float ---> 4 bytes and Doublt ---> 8 bytes.
    ...
    // Now i want to scheck the size of an variabel so we can use sizeof();
